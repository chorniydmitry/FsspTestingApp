...
public class TestController extends ControllerWithTimer implements ActionListener, KeyListener{
	...
	public TestController(TestDialog testDialog, TestingProcess testingProcess, int selectedLevel) {
	...
	private void doFinishAction() {
		testDialog.dispose();
		testingProcess.saveResultsToDB();
	}
	
	private void doNextUnansweredAction() {
		//testDialog.switchPanels();
	}
	...
	private void switchQuestion(int where) {
		setUserChoise();
		int goToQuestionIndex = testingProcess.getCurrentQuestionIndex() + where;
		if(goToQuestionIndex >= testingProcess.getQuestions().size() || goToQuestionIndex < 0)
			return;
		clearRb();
		testingProcess.setCurrentQuestionIndex(goToQuestionIndex);
		showQuestionContents(goToQuestionIndex);
	...
	private void showQuestion(int index) {
		...
		testDialog.getLblQuestionInfo().setText("Вопрос № " + (index + 1) + " из " + testingProcess.getQuestions().size());
	...
	private void showAnswers(int index) {
		...
		for (int i = 0; i < answers.size(); i++) {
			...
			testDialog.getRbAnswers().get(i).setText("<html><p style=\"width:" + width + "px\">"+answers.get(i).getTitle()+"</p></html>");
		}
	...
	private void setOnCloseListener() {
		testDialog.addWindowListener(new java.awt.event.WindowAdapter() {
		    ...
		    public void windowClosed(java.awt.event.WindowEvent windowEvent) {
		    	...
		    	dialog.setCaptions(testingProcess.getCorrectAnswersAmount(),
		    			testingProcess.getQuestions().size(),
		    			testingProcess.getMarkPercent(),
		    			testingProcess.getMarkOneToFive(),
		    			testingProcess.getMarkText(), 
		    			testingProcess.getMarkLetter());
